...
for model in models:
    ...
    f = open("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/Single_BH/" + model + "_outputs.txt", "a")
    ...
    L = [ "\n # of sys.: ",str(len(mP_innms)),  "\n # of sys. undergo CE: ", str(len(mP_ins)), "\n # of sys. undergo CE more than once: ",str(np.sum(maskCEMult)),  
        ...
        "\n # of unbound (1 BH) binaries in all sys.: ", str(np.sum(maskSPUnbound)), "\n # of unbound (1 BH) binaries in CE pos sys.: ", str(np.sum(maskSPUnbound_CEpos)), 
        "\n $%$ of unbound  (1 BH) binaries in all sys.: ", str(np.sum(maskSPUnbound)*100/len(maskSPUnbound)), "\n $%$ of unbound (1 BH) binaries in CE pos sys.: ", str(np.sum(maskSPUnbound_CEpos)*100/len(maskSPUnbound_CEpos)),  
        "\n # of double compact obj. (1 BH) in all sys.: ", str(np.sum(maskSPDCO)),  "\n # of double compact obj. (1 BH) in CE pos sys.: ", str(np.sum(maskSPDCO_CEpos)), 
        "\n $%$ of double compact obj. (1 BH) in all sys.: ", str(np.sum(maskSPDCO)*100/len(maskSPDCO)),  "\n $%$ of double compact obj. (1 BH) in CE pos sys.: ", str(np.sum(maskSPDCO_CEpos)*100/len(maskSPDCO_CEpos)), 
        "\n # of mergers (1 BH) in all sys.: ", str(np.sum(maskSPMerg)),  "\n # of (1 BH) mergers in CE pos sys.: ", str(np.sum(maskSPMerg_CEpos)),  
        "\n $%$ of mergers (1 BH) in all sys.: ", str(np.sum(maskSPMerg)*100/len(maskSPMerg)), "\n $%$ of (1 BH) mergers in CE pos sys.: ", str(np.sum(maskSPMerg_CEpos)*100/len(maskSPMerg_CEpos)), 
        "\n # of bound binaries (1 BH) in all sys.: ", str(np.sum(maskSPBoundBinary)), "\n # of bound binaries (1 BH) in CE pos sys.: ", str(np.sum(maskSPBoundBinary_CEpos)),
        "\n $%$ of bound binaries (1 BH) in all sys.: ", str(np.sum(maskSPBoundBinary)*100/len(maskSPBoundBinary)), "\n $%$ of bound binaries (1 BH) in CE pos sys.: ", str(np.sum(maskSPBoundBinary_CEpos*100/len(maskSPBoundBinary_CEpos)))
        ]
