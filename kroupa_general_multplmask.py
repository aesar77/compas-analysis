...
for model in models:
    ...
    f = open("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + model + "_general_outputs.txt", "a")
    ...
    #IMPORTANT: mask_unique is constructed in a way to select unique seeds from CE[seeds], if a seed has duplicates in CE[seeds] then it chooses the latest one (the one with the most CE event count)
        ...
        #       " \n Number of systems undergo CE more than once: ",(np.sum(maskCEpst)), " \n Percentage of systems undergo CE: ",(np.sum(maskCEpst)*100/len(seedsSP)))
        ...
        maskSPdco = (status == 14)
        maskSPmrgr = (status == 9) 
        maskSPunb_CEpos = (status*mask == 14) 
        maskSPdco_CEpos = (status*mask == 11) 
        maskSPmrgr_CEpos = (status*mask == 9) 
    ...
    arrays_to_save = [ mP_innms,  mP_ins,  mP_preCEs, mP_pstCEs, mS_innms,  mS_ins, mS_preCEs, mS_pstCEs, semax_innms,  semax_ins, semax_preCEs, semax_pstCEs,
                      maskpreCEBH,maskpstCEBH,maskpreCEBBH,maskpstCEBBH,maskSPBH , maskSPBBH,maskSPUnbound,maskSPDCO,maskSPMerg, maskSPBH_CEpos,
                      maskSPBBH_CEpos,maskSPUnbound_CEpos,maskSPDCO_CEpos,maskSPMerg_CEpos,maskCEMult,maskSPBoundBinary,maskSPBoundBinary_CEpos]
    filenames =      ["mP_innms","mP_ins","mP_preCEs","mP_pstCEs","mS_innms","mS_ins","mS_preCEs","mS_pstCEs","semax_innms","semax_ins","semax_preCEs","semax_pstCEs",
                     "maskpreCEBH","maskpstCEBH","maskpreCEBBH","maskpstCEBBH","maskSPBH ","maskSPBBH","maskSPUnbound","maskSPDCO","maskSPMerg","maskSPBH_CEpos",
                     "maskSPBBH_CEpos","maskSPUnbound_CEpos","maskSPDCO_CEpos","maskSPMerg_CEpos","maskCEMult","maskSPBoundBinary","maskSPBoundBinary_CEpos"]
    i = 0
    for f in arrays_to_save:
        name = filenames[i]
        np.savetxt("/data/a.saricaoglu/Files/" + model + "/General/" +  str(c.strftime("%m.%d")) + "/" + name + "_" + str(c.strftime("%m.%d")) + ".txt", f)
        i+=1
    ...
    # plt.xlabel("Initial Primary Mass [$M_\odot$]")
    # plt.ylabel("Systems")
    # plt.yscale("log")
    # #plt.xscale("log")
    # plt.legend(["All","Undergoing CE"], loc='upper right')

    # plt.subplot(3,3,2)
    # plt.suptitle(model+ " IMF")
    # plt.hist([mS_innms, mS_ins], bins=40, log=True)
    # plt.xlabel("Initial Secondary Mass [$M_\odot$]")
    # plt.ylabel("Systems")
    # plt.yscale("log")
    # #plt.xscale("log")
    ...
    # #plt.xscale("log")
    # plt.yscale("log")
    # plt.legend(["$M_p$ ","$M_s$"], loc='upper right')

    # plt.subplot(3,3,5)
    # plt.hist([ mP_preCEs,mS_preCEs], bins=40, log=True)
    # plt.xlabel("Mass before common envelope [$M_\odot$]")
    # plt.ylabel("CE positive Systems")
    # #plt.xscale("log")
    # plt.yscale("log")
    ...
    # plt.xlabel("Mass after common envelope [$M_\odot$]")
    # plt.ylabel("CE positive Systems")
    # #plt.xscale("log")
    # plt.yscale("log")
    ...
    # #plt.xscale("log")
    # plt.ylabel("CE positive Systems")
    # plt.legend(["Initial", "Before common envelope"])
    ...
    # #plt.xscale("log")
    # plt.ylabel("CE positive Systems")
    # plt.legend(["Before common envelope", "After common envelope"])

    # plt.subplot(3,3,9)
    # plt.hist([semax_ins, semax_pstCEs],bins=40, log=True)
    # plt.xlabel("Semimajor axis [AU]")
    # plt.yscale("log")
    # #plt.xscale("log")
    # plt.ylabel("CE positive Systems")
    # plt.legend(["Initial", "After common envelope"])
    ...
    # mP_innms_scale= np.logspace(np.log10(1.0), np.log10(max(mP_innms)), 50)
    # mS_innms_scale= np.logspace(np.log10(1.0), np.log10(max(mS_innms)), 50)

    # mP_ins_scale = np.logspace(np.log10(1.0), np.log10(max(mP_ins)), 50)
    # mS_ins_scale = np.logspace(np.log10(1.0), np.log10(max(mS_ins)), 50)

    # mP_preCEs_scale = np.logspace(np.log10(1.0), np.log10(max(mP_preCEs)), 50)
    # mS_preCEs_scale = np.logspace(np.log10(1.0), np.log10(max(mS_preCEs)), 50)

    # mP_pstCEs_scale = np.logspace(np.log10(1.0), np.log10(max(mP_pstCEs)), 50)
    # mS_pstCEs_scale = np.logspace(np.log10(1.0), np.log10(max(mS_pstCEs)), 50)

    # semax_innms_scale = np.logspace(np.log10(1.0), np.log10(max(semax_innms)), 50)
    # semax_ins_scale = np.logspace(np.log10(1.0), np.log10(max(semax_ins)), 50)
   


    # plt.subplot(3,3,1)
    # # plt.suptitle(model+ " IMF, Population size: " + str(len((mP_innms))) + "\n Evolution Status vs Masses in CE Positive Systems")
    # plt.suptitle("Evolution Status vs Masses in CE Positive Systems")
    # plt.hist2d(ma.masked_array(mP_ins, mask=maskSPBH_CEpos), ma.masked_array(mS_ins, mask=maskSPBH_CEpos),bins=(mP_ins_scale, mS_ins_scale))
    # plt.xlabel("$M_p$ initial [$M_{\odot}$]")
    # plt.ylabel("$M_s$ initial [$M_{\odot}$]")
    # plt.yscale("log")01.23
    # plt.xscale("log")
    # plt.colorbar(label = "Black Hole occurance")

    # plt.subplot(3,3,2)
    # plt.hist2d(ma.masked_array(mP_preCEs,mask=maskpreCEBH), ma.masked_array(mS_preCEs, mask=maskpreCEBH),bins=(mP_preCEs_scale, mS_preCEs_scale))
    # plt.xlabel("$M_p$ before CE [$M_{\odot}$]")
    # plt.ylabel("$M_s$ before CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # plt.colorbar(label = "Black Hole occurance")

    # plt.subplot(3,3,3)
    # plt.hist2d(ma.masked_array(mP_pstCEs, mask=maskpstCEBH), ma.masked_array(mS_pstCEs, mask=maskpstCEBH),bins=(mP_pstCEs_scale, mS_pstCEs_scale))
    # plt.xlabel("$M_p$ after CE [$M_{\odot}$]")
    # plt.ylabel("$M_s$ after CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # plt.colorbar(label = "Black Hole occurance")

    # plt.subplot(3,3,4)
    # plt.hist2d(ma.masked_array(mP_ins, mask= maskSPBBH_CEpos), ma.masked_array(mS_ins, mask=maskSPBBH_CEpos),bins=(mP_ins_scale, mS_ins_scale))
    # plt.xlabel("$M_p$ initial [$M_{\odot}$]")
    # plt.ylabel("$M_s$ initial CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # plt.colorbar(label = "Binary Black Hole occurance")

    # plt.subplot(3,3,5)
    # plt.hist2d(ma.masked_array(mP_preCEs, mask=maskpreCEBBH),ma.masked_array(mS_preCEs, mask=maskpreCEBBH),bins=(mP_preCEs_scale, mS_preCEs_scale))
    # plt.xlabel("$M_p$ before CE [$M_{\odot}$]")
    # plt.ylabel("$M_s$ before CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    ...
    # plt.subplot(3,3,6)
    # plt.hist2d(ma.masked_array(mP_pstCEs, mask=maskpstCEBBH),ma.masked_array(mS_pstCEs, mask=maskpstCEBBH),bins=(mP_pstCEs_scale, mS_pstCEs_scale))
    # plt.xlabel("$M_p$ after CE [$M_{\odot}$]")
    # plt.ylabel("$M_s$ after CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    ...
    # plt.subplot(3,3,7)
    # plt.hist2d(ma.masked_array(mP_ins, mask=maskSPDCO_CEpos), ma.masked_array(mS_ins, mask=maskSPDCO_CEpos),bins=(mP_ins_scale, mS_ins_scale))
    # plt.xlabel("$M_p$ initial [$M_{\odot}$]")
    # plt.ylabel("$M_s$ initial [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # plt.colorbar(label = "DCO occurance")

    # plt.subplot(3,3,8)
    # plt.hist2d(ma.masked_array(mP_ins, mask=maskSPUnbound_CEpos), ma.masked_array(mS_ins, mask=maskSPUnbound_CEpos),bins=(mP_ins_scale, mS_ins_scale))
    # plt.xlabel("$M_p$ initial [$M_{\odot}$]")
    # plt.ylabel("$M_s$ initial [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # plt.colorbar(label = "Unbound Binary occurance")

    # plt.subplot(3,3,9)
    # plt.hist2d(ma.masked_array(mP_ins, mask=maskSPMerg_CEpos), ma.masked_array(mS_ins, mask=maskSPMerg_CEpos),bins=(mP_ins_scale, mS_ins_scale))
    # plt.xlabel("$M_p$ initial [$M_{\odot}$]")
    # plt.ylabel("$M_s$ initial [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # plt.colorbar(label = "Merger occurance")


    # plt.subplots_adjust(top=0.93, bottom=0.07, left=0.1, right=0.9, hspace=0.32, wspace=0.45)
    # plt.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + model + "_2dhist_" + current_time + ".png")
    # plt.close()
    ...
    # # plt.hist2d(semax_innms, mP_innms, bins=(semax_innms_scale, mP_innms_scale))
    # # plt.hist2d(semax_ins, mP_ins, bins=(semax_innms_scale, mP_innms_scale))
    # hist1 = plt.hexbin(semax_innms, mP_innms, gridsize=30, alpha=0.5, cmap='Reds', xscale="log", yscale="log", bins="log")
    # hist2 = plt.hexbin(semax_ins, mP_ins, gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.colorbar(hist2, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_p$ initial [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # #plt.legend(["All","Undergoing CE"])

    # plt.subplot(3,3,2)
    # #plt.scatter(semax_innms, mS_innms ,marker='.')
    # #plt.scatter(semax_ins, mP_ins ,marker='.')
    # # plt.hist2d(semax_innms, mS_innms,bins=(semax_innms_scale, mS_innms_scale))
    # # plt.hist2d(semax_ins, mS_ins, bins=(semax_innms_scale, mS_innms_scale))    
    # hist1 = plt.hexbin(semax_innms, mS_innms, gridsize=30, alpha=0.5, cmap='Reds', xscale="log", yscale="log", bins="log")
    # hist2 = plt.hexbin(semax_ins, mS_ins, gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.colorbar(hist2, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_s$ initial [$M_{\odot}$]")
    ...
    # # plt.hist2d(semax_innms, np.add(mS_innms, mP_innms) ,marker='.')
    # # plt.hist2d(semax_ins, np.add(mS_ins, mP_ins) ,marker='.')
    # hist1 = plt.hexbin(semax_innms, np.add(mS_innms, mP_innms), gridsize=30, alpha=0.5, cmap='Reds', xscale="log", yscale="log", bins="log")
    # hist2 = plt.hexbin(semax_ins,np.add(mS_ins, mP_ins), gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.colorbar(hist2, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_s + M_p$ initial [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # #plt.legend(["All Systems","Systems undergoing CE"])
    ...
    # # plt.hist2d(semax_preCEs, mP_preCEs ,marker='.')
    # hist1 = plt.hexbin(semax_preCEs, mP_preCEs, gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_p$ before CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")

    # plt.subplot(3,3,5)
    # #plt.scatter(semax_preCEs, mS_preCEs ,marker='.')
    # # plt.hist2d(semax_preCEs, mS_preCEs ,marker='.')
    # hist1 = plt.hexbin(semax_preCEs, mS_preCEs, gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_s$ before CE [$M_{\odot}$]")
    # plt.yscale("log")
    ...
    # # plt.hist2d(semax_preCEs, np.add(mS_preCEs,mS_preCEs) ,marker='.')
    # hist1 = plt.hexbin(semax_preCEs, np.add(mS_preCEs,mS_preCEs), gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_s + M_p$ before CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")

    # plt.subplot(3,3,7)
    # #plt.scatter(semax_pstCEs, mP_pstCEs,marker='.')
    # # plt.hist2d(semax_pstCEs, mP_pstCEs,marker='.')
    # hist1 = plt.hexbin(semax_pstCEs, mP_pstCEs, gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_p$ after CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    ...
    # # plt.hist2d(semax_pstCEs, mS_pstCEs,marker='.')
    # hist1 = plt.hexbin(semax_pstCEs, mS_pstCEs, gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_s$ after CE [$M_{\odot}$]")
    # plt.yscale("log")
    ...
    # # plt.hist2d(semax_pstCEs, np.add(mS_pstCEs, mP_pstCEs) ,marker='.')
    # hist1 = plt.hexbin(semax_pstCEs, np.add(mS_pstCEs, mP_pstCEs), gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.xlabel("Semimajor Axis [AU]")
    # plt.ylabel("$M_s + M_p$ after CE [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    ...
    # hist1 = plt.hexbin(ma.masked_array(mP_innms, mask=maskSPBoundBinary),ma.masked_array(mS_innms, mask=maskSPBoundBinary), gridsize=30, alpha=0.5, cmap='Reds', xscale="log", yscale="log", bins="log")
    # hist2 = plt.hexbin(ma.masked_array(mP_ins, mask=maskSPBoundBinary_CEpos),ma.masked_array(mS_ins, mask=maskSPBoundBinary_CEpos), gridsize=30, alpha=0.5, cmap='Blues', xscale="log", yscale="log", bins="log")
    # plt.colorbar(hist1, orientation='vertical')
    # plt.colorbar(hist2, orientation='vertical')
    # plt.xlabel("Initial Primary Mass (BH component) [$M_{\odot}$]")
    # plt.ylabel("Initial Secondary Mass (BH/CO/Star component) [$M_{\odot}$]")
    # plt.yscale("log")
    # plt.xscale("log")
    # # plt.axvline(0.1, label="0.1 [$M_{\odot}$]")
    # # plt.axvline(1000, label="1000 [$M_{\odot}$]")
    # # plt.legend(["All systems", "Systems undergoing CE"])
