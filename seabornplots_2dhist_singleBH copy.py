...
for model in models:
    ...
    pathToData = '/data/a.saricaoglu/Files/' + model  + "/03.25/"
    ...
    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax"],SPdf_singleBH["mP"]), bins=20)
    df = pd.DataFrame({
    "mP_innms": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_innms": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    vals = df.pivot(index="mP_innms", columns="semax_innms", values="frequency")
    ...
    ax.set_title("Single BH only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap1_" + current_time + ".png",   bbox_inches='tight')
    plt.close()
    
    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_pr"],(SPdf_singleBH["mP_pr"])), bins=20)
    df = pd.DataFrame({
    "mS_innms": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_innms": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    vals = df.pivot(index="mS_innms", columns="semax_innms", values="frequency")
    f, ax = plt.subplots(figsize=(27, 24))
    xlabl = ["{:.2f}".format(10**x) for x in vals.columns]
    ylabl = ["{:.1f}".format(10**y) for y in vals.index]
    hmapplot = sns.heatmap(vals, annot=True,fmt=".0f", linewidths=.1, ax=ax, xticklabels=xlabl, yticklabels=ylabl)
    ...
    ax.set_xlabel("Semimajor Axis inital [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("Single BH ($M_p$) only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap2_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_sc"],SPdf_singleBH["mP_sc"]), bins=20)
    df = pd.DataFrame({
    "mP_preCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_preCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    vals = df.pivot(index="mP_preCEs", columns="semax_preCEs", values="frequency")
    f, ax = plt.subplots(figsize=(27, 24))
    xlabl = ["{:.2f}".format(10**x) for x in vals.columns]
    ylabl = ["{:.1f}".format(10**y) for y in vals.index]
    hmapplot = sns.heatmap(vals, annot=True,fmt=".0f", linewidths=.1, ax=ax, xticklabels=xlabl, yticklabels=ylabl)
    ...
    ax.invert_yaxis()
    ax.tick_params(axis='both', which='major', labelsize=20)
    ax.set_xlabel("Semimajor Axis [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$ [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("Single BH ($M_s$) only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap3_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax"],SPdf_singleBH["mS"]), bins=20)
    df = pd.DataFrame({
    "mS_preCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_preCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    vals = df.pivot(index="mS_preCEs", columns="semax_preCEs", values="frequency")
    f, ax = plt.subplots(figsize=(27, 24))
    xlabl = ["{:.2f}".format(10**x) for x in vals.columns]
    ylabl = ["{:.1f}".format(10**y) for y in vals.index]
    hmapplot = sns.heatmap(vals, annot=True,fmt=".0f", linewidths=.1, ax=ax, xticklabels=xlabl, yticklabels=ylabl)
    ...
    ax.set_title("Single BH only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap4_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_pr"],SPdf_singleBH["mS_pr"]), bins=20)
    df = pd.DataFrame({
    "mP_pstCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_pstCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    vals = df.pivot(index="mP_pstCEs", columns="semax_pstCEs", values="frequency")
    f, ax = plt.subplots(figsize=(27, 24))
    xlabl = ["{:.2f}".format(10**x) for x in vals.columns]
    ylabl = ["{:.1f}".format(10**y) for y in vals.index]
    hmapplot = sns.heatmap(vals, annot=True,fmt=".0f", linewidths=.1, ax=ax, xticklabels=xlabl, yticklabels=ylabl)
    ...
    ax.set_ylabel("$M_s$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("Single BH ($M_p$) only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap5_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_sc"],SPdf_singleBH["mS_sc"]), bins=20)
    df = pd.DataFrame({
    "mS_pstCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_pstCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    vals = df.pivot(index="mS_pstCEs", columns="semax_pstCEs", values="frequency")
    f, ax = plt.subplots(figsize=(27, 24))
    xlabl = ["{:.2f}".format(10**x) for x in vals.columns]
    ylabl = ["{:.1f}".format(10**y) for y in vals.index]
    hmapplot = sns.heatmap(vals, annot=True,fmt=".0f", linewidths=.1, ax=ax, xticklabels=xlabl, yticklabels=ylabl)
    ...
    ax.set_title("Single BH ($M_s$) only",  fontsize=40, pad=30)
    ax.set_ylabel("$M_s$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap6_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_CE"],SPdf_singleBH["mP_CE"]), bins=20)
    df = pd.DataFrame({
    "mP_innms": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_innms": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis inital [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positve single BH only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap11_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_pr_CE"], SPdf_singleBH["mP_pr_CE"]), bins=20)
    df = pd.DataFrame({
    "mS_innms": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_innms": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis inital [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive single BH ($M_p$) only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap22_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_sc_CE"],SPdf_singleBH["mP_sc_CE"]), bins=20)
    df = pd.DataFrame({
    "mP_preCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_preCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$ [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive single BH ($M_s$) only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap33_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_CE"],SPdf_singleBH["mS_CE"]), bins=20)
    df = pd.DataFrame({
    "mS_preCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_preCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_s$ [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive single BH only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap44_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_pr_CE"],SPdf_singleBH["mS_pr_CE"]), bins=20)
    df = pd.DataFrame({
    "mP_pstCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_pstCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_ylabel("$M_s$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive Single BH ($M_p$) only",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap55_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(SPdf_singleBH["semax_sc_CE"],SPdf_singleBH["mS_sc_CE"]), bins=20)
    df = pd.DataFrame({
    "mS_pstCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_pstCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_title("CE positive single BH ($M_s$) only",  fontsize=40, pad=30)
    ax.set_ylabel("$M_s$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap66_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax"],DCdf_singleBH["mP"]), bins=20)
    df = pd.DataFrame({
    "mP_innms": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_innms": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("Single BH only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap7_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_pr"], (DCdf_singleBH["mP_pr"])), bins=20)
    df = pd.DataFrame({
    "mS_innms": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_innms": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("Single BH ($M_p$) only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap8_" + current_time + ".png",   bbox_inches='tight')

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_sc"],DCdf_singleBH["mP_sc"]), bins=20)
    df = pd.DataFrame({
    "mP_preCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_preCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$ [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("Single BH ($M_s$) only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap9_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax"],DCdf_singleBH["mS"]), bins=20)
    df = pd.DataFrame({
    "mS_preCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_preCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_ylabel("$M_s$ [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("Single BH only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap10_" + current_time + ".png",   bbox_inches='tight')
    plt.close()    

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_pr"],DCdf_singleBH["mS_pr"]), bins=20)
    df = pd.DataFrame({
    "mP_pstCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_pstCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_ylabel("$M_s$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("Single BH ($M_p$) only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap12_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_sc"],DCdf_singleBH["mS_sc"]), bins=20)
    df = pd.DataFrame({
    "mS_pstCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_pstCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_title("Single BH ($M_s$) only (DCO)",  fontsize=40, pad=30)
    ax.set_ylabel("$M_s$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap13_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_CE"],DCdf_singleBH["mP_CE"]), bins=20)
    df = pd.DataFrame({
    "mP_innms": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_innms": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive single BH only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap77_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_pr_CE"], DCdf_singleBH["mP_pr_CE"]), bins=20)
    df = pd.DataFrame({
    "mS_innms": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_innms": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis inital [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive single BH ($M_p$) only (DCO=",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap88_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_sc_CE"],DCdf_singleBH["mP_sc_CE"]), bins=20)
    df = pd.DataFrame({
    "mP_preCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_preCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_xlabel("Semimajor Axis [AU]",  fontsize=40, labelpad=25)
    ax.set_ylabel("$M_p$ [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive single BH ($M_s$) only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap99_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_CE"],DCdf_singleBH["mS_CE"]), bins=20)
    df = pd.DataFrame({
    "mS_preCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_preCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_ylabel("$M_s$ [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive single BH only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap100_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_pr_CE"],DCdf_singleBH["mS_pr_CE"]), bins=20)
    df = pd.DataFrame({
    "mP_pstCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_pstCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_ylabel("$M_s$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    ax.set_title("CE positive single  BH ($M_p$) only (DCO)",  fontsize=40, pad=30)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap111_" + current_time + ".png",   bbox_inches='tight')
    plt.close()

    values, xbins, ybins = np.histogram2d(*mylog(DCdf_singleBH["semax_sc_CE"],DCdf_singleBH["mS_sc_CE"]), bins=20)
    df = pd.DataFrame({
    "mS_pstCEs": np.repeat(xbins[:-1], len(ybins)-1),
    "semax_pstCEs": np.tile(ybins[:-1], len(xbins)-1),
    'frequency': values.T.flatten()})
    ...
    ax.set_title("CE positive single  BH ($M_s$) only (DCO)",  fontsize=40, pad=30)
    ax.set_ylabel("$M_s$  [$M_{\odot}$]",  fontsize=40, labelpad=25)
    f.savefig("/data/a.saricaoglu/Plots/" + str(c.strftime("%m.%d")) + "/" + mode  + "_" +  model + "_heatmap122_" + current_time + ".png",   bbox_inches='tight')
    plt.close()