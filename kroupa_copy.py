...
for run in runs:
    ...
    output_dir = run + "/" + out[0]
    try:
        data = h5.File(output_dir)
...
for Data in data_outputs:
    ...
    maskCE        =  (events == 1)
    maskedSeedsCE = seedsCE[maskCE]
    print(len(maskedSeedsCE))
    ### Create seedsOfInterest dictionary
    seedsOfInterest   = {'BSE_System_Parameters':      seedsSP,
                        'BSE_Common_Envelopes': maskedSeedsCE,
                        }


### Don't forget to close the original h5 data file
    Data.close()
    h5rewrite.reduceH5(pathToOld = output_dirs[m], pathToNew = pathToData + "Reduced/COMPAS_Output_reduced.h5",
                        dictColumns=columnsOfInterest, dictSeeds=seedsOfInterest)
    
    data = h5.File(pathToData + "Reduced/COMPAS_Output_reduced.h5")
    SPr = data['BSE_System_Parameters']
    CEr = data['BSE_Common_Envelopes']
    ...
    mP_in = SPr['Mass@ZAMS(1)'][()]
    mS_in = SPr['Mass@ZAMS(2)'][()]

    mP_preCE = CEr["Mass(1)<CE"][()]
    mS_preCE = CEr["Mass(2)<CE"][()]

    mP_pstCE = CEr["Mass(1)>CE"][()]
    mS_pstCE = CEr["Mass(2)>CE"][()]

    mask= np.in1d(seedsSP, seedsCEm)
    

    semax_in = SPr['SemiMajorAxis@ZAMS'][()]
    semax_preCE = CEr["SemiMajorAxis<CE"][()]
    semax_pstCE = CEr["SemiMajorAxis>CE"][()]
    ...
    data.close()
    print(len(mP_in), len(mP_in[mask]), len(mP_preCE), len(mS_pstCEs))
...
plt.savefig("/data/a.saricaoglu/Plots/kroupa_hist.png")
plt.show()
...
plt.subplots_adjust(top=0.93, bottom=0.07, left=0.045, right=0.955, hspace=0.32, wspace=0.235)
plt.savefig("/data/a.saricaoglu/Plots/kroupa_scatter.png")
plt.show()
