...
for IMF in IMFs:
    ...
    f = open("/data/a.saricaoglu/Files/Kroupa/" + IMF + "_" + str(c.strftime("%m.%d")) +  "_general_outputs.txt", "a")
...
## may add eccentricity later on
    ...
    inc = np.random.uniform(0,180,len(DCs))
    MASKDCCOSI = (np.abs(np.cos(inc)) <= 2.22819667e-6)
    ...
    L = ["\n Number of systems: ",str(len(MASSZAMSSP1)),  "\n Number of systems forming DCO (via SP status): ", str(np.sum(MASKSPdco)), "\n Number of systems forming DCO (via DCO mass): ", str(len(MASSDC1)), 
        ...
        "\n Number of primary HeWD (DC) : ", str(np.sum([STELLARTYPEDC1 == 10 ])),
        "\n Number of primary COWD (DC) : ", str(np.sum([STELLARTYPEDC1 == 11 ])),
        "\n Number of primary ONeWD (DC) : ", str(np.sum([STELLARTYPEDC1 == 12 ])),
        "\n Number of secondary HeWD (DC) : ", str(np.sum([STELLARTYPEDC2 == 10 ])),
        "\n Number of secondary COWD (DC) : ", str(np.sum([STELLARTYPEDC2 == 11 ])),
        "\n Number of secondary ONeWD (DC) : ", str(np.sum([STELLARTYPEDC2 == 12 ])),      
        "\n Number of primary HeWD (SP) : ", str(np.sum([STELLARTYPESP1 == 10 ])),
        "\n Number of primary COWD (SP) : ", str(np.sum([STELLARTYPESP1 == 11 ])),
        "\n Number of primary ONeWD (SP) : ", str(np.sum([STELLARTYPESP1 == 12 ])),
        "\n Number of secondary HeWD (SP) : ", str(np.sum([STELLARTYPESP2 == 10 ])),
        "\n Number of secondary COWD (SP) : ", str(np.sum([STELLARTYPESP2 == 11 ])),
        "\n Number of secondary ONeWD (SP) : ", str(np.sum([STELLARTYPESP2 == 12 ])),         
        ]
